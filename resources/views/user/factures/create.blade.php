...
@section('title', 'Ajouter une facture - Gestion des loyers')
    ...
    <div class="max-w-3xl mx-auto">
        <div class="bg-white shadow-sm rounded-lg overflow-hidden">
            <div class="p-6 border-b border-gray-200">
                <h2 class="text-lg font-medium text-gray-900">Ajouter une nouvelle facture</h2>
                <p class="mt-1 text-sm text-gray-600">Remplissez les détails de la facture ci-dessous.</p>
            </div>

            <form action="{{ route('user.factures.store') }}" method="POST" class="p-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="type" name="type" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                            <option value="loyer" {{ old('type') == 'loyer' ? 'selected' : '' }}>Loyer</option>
                            <option value="charges" {{ old('type') == 'charges' ? 'selected' : '' }}>Charges</option>
                            <option value="reparations" {{ old('type') == 'reparations' ? 'selected' : '' }}>Réparations</option>
                    ...
                    <div>
                        ...
                        <div class="mt-1 relative rounded-md shadow-sm">
                            <input type="number" name="montant" id="montant" step="0.01" min="0" value="{{ old('montant') }}" required
                    ...
                    <div>
                        ...
                        <input type="date" name="date_emission" id="date_emission" value="{{ old('date_emission', now()->format('Y-m-d')) }}" required
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        @error('date_emission')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date" name="date_echeance" id="date_echeance" value="{{ old('date_echeance', now()->addMonth()->format('Y-m-d')) }}" required
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        @error('date_echeance')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
